%RUN_VOICE_LINK_MATLAB_SIMULINK Independent MATLAB/Simulink validation for voice-link.
%
% This script reproduces the voice-link link-side voice-availability and
...
fprintf("voice-link MATLAB/Simulink validation\n");
fprintf("Project root: %s\n", cfg.rootDir);
fprintf("Output dir:   %s\n\n", cfg.outputDir);

voiceRows = voice_link_availability_matlab(cfg);
outageRows = voice_link_outage_capacity_matlab(cfg);
...
if usejava("jvm") && license("test", "Simulink") && exist("simulink", "file") == 2
    ...
    try
        ...
        simRows = voice_link_run_simulink_cosim(cfg, modelName);
        simStatus = "completed";
    catch simErr
        simStatus = "blocked";
...
compareRows = voice_link_compare_with_python(cfg, voiceRows, outageRows, simRows);
voice_link_export_summary(cfg, voiceRows, outageRows, simRows, compareRows, simStatus);
...
fprintf("- %s\n", fullfile(cfg.outputDir, "matlab_voice_link_validation_summary.md"));
