%RUN_VOICE_LINK_MATLAB_DESKTOP_JOB Run voice-link validation from MATLAB Desktop.
%
...
diary(logPath);
fprintf('voice-link MATLAB Desktop GUI job started: %s\n', datestr(now, 31));

try
    run_voice_link_matlab_simulink;
    fid = fopen(statusPath, 'w');
    fprintf(fid, 'completed\n%s\n', datestr(now, 31));
    fclose(fid);
    fprintf('voice-link MATLAB Desktop GUI job completed: %s\n', datestr(now, 31));
catch ME
    fprintf(2, '\nvoice-link MATLAB Desktop GUI job failed: %s\n', ME.message);
