function rows = voice_link_run_simulink_cosim(cfg, modelName, thresholdRows, strict)
%VOICE_LINK_RUN_SIMULINK_COSIM Run the generated Simulink model over scenarios.
...
modelMode = voice_link_detect_simulink_model_mode(modelName);
...
for i = 1:height(cfg.scenarios)
    for j = 1:numel(cfg.voice.rateBps)
        ...
        thresholdDb = voice_link_lookup_voice_threshold(cfg, thresholdRows, rateBps, j);
        thresholdSource = voice_link_threshold_source(thresholdRows);
        ...
        assignin("base", "voice_link_reference_cfg", cfg);

        if strcmp(modelMode, 'precomputed_metrics')
            simPrecomputedMetrics = voice_link_simulink_scenario([i; cfg.nMcVoice; cfg.seed; rateBps; thresholdDb]);
        ...
        try
            ...
            warning("voice_link:simulinkRuntimeFallback", ...
                "MATLAB Function block simulation failed (%s). Rebuilding as a precomputed-metrics Simulink orchestration model.", ...
                simErr.message);
            voice_link_switch_to_precomputed_model(modelName);
            modelMode = 'precomputed_metrics';
            simPrecomputedMetrics = voice_link_simulink_scenario([i; cfg.nMcVoice; cfg.seed; rateBps; thresholdDb]);
            assignin("base", "sim_precomputed_metrics", simPrecomputedMetrics);
            simOut = sim(modelName, "StopTime", "1");
        end
        raw = voice_link_get_simulink_output(simOut);
        values = voice_link_last_simulink_row(raw);

        sc = cfg.scenarios(i, :);
        row = table( ...
            sc.scenario_key, sc.scenario_model_key, sc.label, ...
...
function values = voice_link_last_simulink_row(raw)
if istable(raw)
    raw = table2array(raw);
end
...
function thresholdDb = voice_link_lookup_voice_threshold(cfg, thresholdRows, rateBps, rateIndex)
...
function source = voice_link_threshold_source(thresholdRows)
...
function raw = voice_link_get_simulink_output(simOut)
...
function modelMode = voice_link_detect_simulink_model_mode(modelName)
...
function voice_link_switch_to_precomputed_model(modelName)
