function rows = voice_link_run_lms_simulink_cosim(cfg, modelName, thresholdRows)
%VOICE_LINK_RUN_LMS_SIMULINK_COSIM Run LMS Simulink model over scenarios.
...
for i = 1:height(cfg.scenarios)
    ...
    if strcmp(modelMode, 'precomputed_metrics')
        lmsPrecomputedMetrics = voice_link_simulink_lms_scenario([i; numFrames; cfg.seed; thresholdDb]);
    ...
    sc = cfg.scenarios(i, :);
    rows = [rows; table( ...
        sc.scenario_key, sc.scenario_model_key, sc.label, cfg.voice.mainRateBps, ...
