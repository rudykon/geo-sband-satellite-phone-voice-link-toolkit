%RUN_VOICE_LINK_MATLAB_CORE_NOJVM Minimal MATLAB core validation for no-JVM batch.
%
...
% string, graphics, and Simulink APIs so the voice-link legacy numerical check can
% still be executed from:
%
%   matlab -nojvm -batch "cd('...\matlab_voice_link'); run_voice_link_core_nojvm"
...
fprintf('voice-link MATLAB no-JVM core validation\n');
...
if exist(pyVoicePath, 'file')
    ...
    for i = 1:numel(scenarioKey)
        pyVal = find_python_voice_availability(txt, scenarioModelKey{i}, 2400.0);
        if ~isnan(pyVal)
