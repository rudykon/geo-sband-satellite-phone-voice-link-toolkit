function rows = voice_link_availability_matlab(cfg, thresholdRows)
%VOICE_LINK_VOICE_AVAILABILITY_MATLAB Reproduce voice-link voice availability.
...
for i = 1:height(cfg.scenarios)
    for j = 1:numel(cfg.voice.rateBps)
        ...
        metrics = voice_link_scenario_metrics(cfg, i, rateBps, thresholdDb, cfg.nMcVoice, cfg.seed);
...
skipFigures = strcmp(getenv('VOICE_LINK_SKIP_FIGURES'), '1');
if ~skipFigures
