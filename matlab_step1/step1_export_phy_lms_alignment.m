function alignRows = step1_export_phy_lms_alignment(cfg)
%STEP1_EXPORT_PHY_LMS_ALIGNMENT Export three-way validation tables.
...
for i = 1:height(matMain)
    ...
    step1Key = string(matMain.step1_key(i));
    pyIdx = strcmp(string(pyVoice.scenario), step1Key) & abs(pyVoice.voice_rate_bps - cfg.voice.mainRateBps) < 1e-9;
    ...
    if any(pyIdx) && any(simIdx) && any(lmsIdx)
        ...
        alignRows = [alignRows; table( ...
            scenarioKey, step1Key, string(matMain.label(i)), ...
...
fprintf(fid, '# Step 1 PHY/LMS MATLAB-Simulink Alignment Summary\n\n');
