function rows = step1_run_lms_channel_timeseries(cfg, thresholdRows)
%STEP1_RUN_LMS_CHANNEL_TIMESERIES LMS frame-level channel simulation.
%
...
thresholdDb = step1_lookup_threshold(thresholdRows, rateBps, cfg.voice.mainThresholdEbN0Db);
ferSlopeDb = 1.25;
...
for i = 1:height(cfg.scenarios)
    ...
    losState = step1_markov_los(numFrames, sc.p_los, sc.p_ll, sc.p_nn);
    shadowDb = step1_correlated_shadowing(numFrames, sc.sigma_db, 0.985);
    smallScaleDb = step1_small_scale_fading(losState, i, numFrames);
    ...
    maxOutageFrames = step1_max_run_length(frameError);
    maxOutageMs = maxOutageFrames * frameDurationMs;
    p95BurstMs = step1_burst_quantile(frameError, frameDurationMs, 0.95);

    rows = [rows; table( ...
        sc.scenario_key, sc.step1_key, sc.label, rateBps, thresholdDb, ...
...
function thresholdDb = step1_lookup_threshold(thresholdRows, rateBps, fallback)
...
function los = step1_markov_los(n, pLosTarget, pLL, pNN)
los = false(n, 1);
...
function shadowDb = step1_correlated_shadowing(n, sigmaDb, rho)
w = randn(n, 1);
shadowDb = zeros(n, 1);
...
function fadingDb = step1_small_scale_fading(losState, scenarioIndex, n)
...
function maxLen = step1_max_run_length(flags)
maxLen = 0;
cur = 0;
for k = 1:numel(flags)
    if flags(k)
...
function qMs = step1_burst_quantile(flags, frameDurationMs, q)
lengths = [];
cur = 0;
for k = 1:numel(flags)
    if flags(k)
